...
import {ACCESS_SERVICE, IAccessService, IEntityUserToken} from './access/types';
...
export class DataService implements IDataService {
  ...
  add<T extends IEntity>(entity: T): Observable<T> {
    ...
    return this.requestFactory.create('add', entity.type, {body: {data: entity.toJsonApi()}}).pipe(
      switchMap((request) => this.requestWithAuth(request)),
      map((entity: T) => entity)
    );
  }
  ...
  list<T extends IEntity>(entity_type: string, params: IQueryParams): Observable<T[]> {
    return this.requestFactory.create('list', entity_type, params).pipe(
      switchMap((request) => this.requestWithAuth<T>(request) as Observable<T[]>),
    );
  }

  protected auth(request: IRequest): Observable<IRequest> {

    return this.access.getToken().pipe(
      switchMap((token: IEntityUserToken) => {
        return token ? of(token) : this.access.getRequest().pipe(
          switchMap((request) => this.request(request))
        );
      }),
      map((token: IEntityUserToken) => this.access.prepareRequest(request, token))
    );
  ...
  protected requestWithAuth<T extends IEntity>(request: IRequest): Observable<T | T[]> {
    return this.auth(request).pipe(
